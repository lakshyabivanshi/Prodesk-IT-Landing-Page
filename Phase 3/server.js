...
app.post('/api/generate',upload.single('resume'), async (req, res) => {
    ...
    try {
        ...
        let userPrompt = `Write a cover letter for ${name}, who is applying for the ${role} position at ${company}. Their core skills are: ${skills}. Keep it under 300 words.`;

        const parts = [{ text: `${systemPrompt}\n\n${userPrompt }`}]

        if(req.file){
            userPrompt += `Also analyze the attached resume PDF and tailor the cover letter to match the candidate actual experience mentioned in it.`;
